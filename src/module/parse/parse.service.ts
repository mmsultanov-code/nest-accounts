...
export class ParseService {
     ...
     * @throws HttpException if an error occurs during the creation process.
     */
    private bulk_create_users = async (users: Array<{ first_name: string; last_name: string; email: string; password: string; avatar: string; role_id: number }>) => {
        const transaction = await this.sequelize.transaction()
        try {
     ...
     */
    async parse_data() {
        const subjects: string[] = ['user', 'permission', 'role', 'account']
